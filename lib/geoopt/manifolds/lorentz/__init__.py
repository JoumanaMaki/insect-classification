...
class Lorentz(Manifold):
    ...
    def expmap(
        self, x: torch.Tensor, u: torch.Tensor, *, norm_tan=False, project=True, dim=-1
    ) -> torch.Tensor:
        if norm_tan is True:
    ...
    def logmap_then_transp0back(self, x, v):
        return math.logmap_then_transp0back(x,v,k=self.k)
    ...
    def origin(
        ...
        return ManifoldTensor(zero_point, manifold=self)
