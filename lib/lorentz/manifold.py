...
class CustomLorentz(Lorentz):
    def __init__(self, k=1.0, learnable=False):
        ...
        self.r = nn.Parameter(torch.tensor(1.2), requires_grad=False)

        self.max_dist = nn.Parameter(
            (1 / 2) * torch.arccosh(torch.tensor(1 / (2 * eps["32"])).to(device=self.k.device) * self.r),
            requires_grad=False)
    ...
    def angle_at_origin(self, x, y):
        ip = math.inner(x, y)
        x0 = x[0]
        y0 = y[0]
        denom = (torch.sqrt(- self.k + self.inner0(x)**2) * torch.sqrt(- self.k + self.inner0(y)**2))
        # Cosine of angle
        cos_theta = (ip + self.k) / denom

        if torch.isnan(cos_theta).sum() > 0:
            print("issue")
...
#
    ...
    def lorentz_scattering_loss(self, Z: torch.Tensor) -> torch.Tensor:
        """
        Distance-based scattering regularization for hyperbolic embeddings in the Lorentz model.
        Penalizes embeddings that collapse into small regions of the hyperbolic space.

        Args:
            Z: Input embeddings on the Lorentz manifold (shape: [batch_size, dim]).
               Last dimension: (t, x_1, ..., x_n), where t^2 - ||x||^2 = k.
            k: Curvature parameter (1/k is the curvature). Must be > 0.

        Returns:
            Scattering loss value (sum of exponential Lorentz distance terms).
        """
        batch_size = Z.shape[0]

        k = self.k

        distances = self.dist(Z, Z.unsqueeze(-2)).clamp(min=0)

        batch_size = Z.shape[0]
        mask = torch.eye(batch_size, dtype=torch.bool, device=Z.device)

        # Compute scattering terms
        exp_terms = torch.exp(-distances / torch.sqrt(k))
        n = exp_terms.shape[0]

        exp_terms_origin = torch.exp(self.dist0(Z)/ torch.sqrt(k)).mean()

        if torch.any(torch.isnan(exp_terms)):
            print("break")

        return 0.2*exp_terms.masked_fill(mask, 0.0).sum()/(n*(n-1)/2) + 0.1*exp_terms_origin  # Account for symmetric pairs
